          ...
          "[UndoRedo][AgentBridge]")
{
    ...
    stack.take_snapshot(
        "Initial project", model, selection, plates, snapshot_data);
    model.add_object();
    plates.create_plate(false);
    const size_t undone_state = stack.take_snapshot(
        "Second edit", model, selection, plates, snapshot_data);
    model.add_object();
    plates.create_plate(false);

    REQUIRE(stack.undo(
        model, selection, plates, snapshot_data, undone_state));
    ...
    const size_t owned_import = stack.take_snapshot(
        "Import model", model, selection, plates, snapshot_data);
