...
#include "GUI.hpp"
#include "Agent/AgentProcessTracker.hpp"
#include "GUI_App.hpp"
...
struct Plater::priv
{
    ...
    std::vector<size_t> load_model_objects(const ModelObjectPtrs& model_objects, bool allow_negative_z = false, bool split_object = false, bool auto_drop = true,
                                           bool suppress_policy_dialogs = false);
    ...
    size_t take_snapshot(const std::string& snapshot_name, UndoRedo::SnapshotType snapshot_type = UndoRedo::SnapshotType::Action);
    ...
    void rollback_to_snapshot(size_t time_to_load);
...
private:
    ...
    void undo_redo_to(std::vector<UndoRedo::Snapshot>::const_iterator it_snapshot,
                      bool discard_current_state = false);
    ...
    Agent::AgentProcessTracker agent_process;
};
    ...
    if (was_running && ! this->background_process.running() && (return_state & UPDATE_BACKGROUND_PROCESS_RESTART) == 0) {
        if (invalidated != Print::APPLY_STATUS_UNCHANGED || this->background_process.is_internal_cancelled())
        {
            ...
            SlicingProcessCompletedEvent evt(EVT_PROCESS_COMPLETED, 0,
                ...
                background_process.run_generation());
                                ...
                                bool show_warnings)
{
    ...
    show_warning_dialog = show_warnings;
...
void Plater::priv::on_slicing_update(SlicingStatusEvent &evt)
{
    ...
    const auto& agent_state = agent_process.snapshot();
    if (agent_state.active && !agent_state.terminal &&
        !agent_process.accepts(evt.generation()))
        return;
    if (evt.status.percent >= 0) {
        const double progress = std::clamp(static_cast<double>(evt.status.percent) / 100.0,
                                           0.0, 1.0);
        agent_process.update_progress(evt.generation(), progress);
    }
...
void Plater::priv::on_process_completed(SlicingProcessCompletedEvent &evt)
{
    ...
    const auto& agent_state = agent_process.snapshot();
    const bool agent_owned = agent_process.accepts(evt.generation());
    if (agent_state.active && !agent_state.terminal && !agent_owned)
        return;
    if (agent_owned && evt.success())
        agent_process.finish(evt.generation(), Agent::AgentProcessOutcome::Succeeded);
    else if (agent_owned && evt.cancelled())
        agent_process.finish(evt.generation(), Agent::AgentProcessOutcome::Cancelled);
    //BBS: add project slice logic
    ...
    if (evt.error()) {
        ...
        if (agent_owned)
            agent_process.finish(evt.generation(), Agent::AgentProcessOutcome::Failed,
                                 message.first);
...
size_t Plater::priv::take_snapshot(const std::string& snapshot_name, const UndoRedo::SnapshotType snapshot_type)
{
    ...
    const size_t snapshot_time = this->undo_redo_stack().take_snapshot(
        snapshot_name, model,
...
void Plater::priv::rollback_to_snapshot(size_t time_to_load)
{
    const std::vector<UndoRedo::Snapshot>& snapshots =
        this->undo_redo_stack().snapshots();
    const auto snapshot = std::lower_bound(
        snapshots.begin(), snapshots.end(), UndoRedo::Snapshot(time_to_load));
    if (snapshot == snapshots.end() || snapshot->timestamp != time_to_load)
        throw std::runtime_error("Owned rollback snapshot does not exist");
    this->undo_redo_to(snapshot, true);
}
...
void Plater::priv::undo_redo_to(
    std::vector<UndoRedo::Snapshot>::const_iterator it_snapshot,
    bool discard_current_state)
{
    ...
    // Do the jump in time.
    const bool restored = discard_current_state ?
        this->undo_redo_stack().rollback_to_snapshot(
            model,
            get_current_canvas3D()->get_canvas_type() == GLCanvas3D::CanvasAssembleView ?
        ...
        (it_snapshot->timestamp < this->undo_redo_stack().active_snapshot_time() ?
...
std::vector<size_t> Plater::load_model_for_agent(const Model& imported)
{
    if (imported.objects.empty())
        return {};
    const size_t rollback_snapshot = p->take_snapshot(_u8L("Import model"));
    if (rollback_snapshot == SIZE_MAX ||
        !p->undo_redo_stack().has_undo_snapshot(rollback_snapshot))
        throw std::runtime_error("Unable to create the model import rollback snapshot");
    try {
        return p->load_model_objects(imported.objects, false, false, true, true);
    } catch (...) {
        // load_model_objects updates the model, object list and plate assignment
        // incrementally. Restore the complete pre-import snapshot on any failure
        // so the controller can report a failed job without a partial scene.
        // Target the owned snapshot directly because interactive undo is guarded
        // in assemble view and may intentionally decline non-gizmo actions.
        p->rollback_to_snapshot(rollback_snapshot);
        throw;
    }
}
...
bool Plater::arrange_for_agent(std::function<void(bool failed, std::string error)> completion)
{
...
bool Plater::export_gcode_for_agent(const boost::filesystem::path& output_path,
                                    std::size_t plate_index)
{
    if (output_path.empty() || p->background_process.is_export_scheduled())
        return false;
    if (plate_index >= static_cast<std::size_t>(p->partplate_list.get_plate_count()))
        return false;
    select_plate(static_cast<int>(plate_index));
    ...
    std::function<void(bool, std::string)> completion)
{
    ...
    try {
        payload = std::make_shared<SavePayload>();
        payload->path = output_path.string();
        if (payload->path.empty() || !boost::algorithm::iends_with(payload->path, ".3mf"))
            throw std::runtime_error("Invalid Orca project save path");

        // Everything below touches live Plater/application state and therefore remains on
        ...
        payload->project.profiles.clear();

        const auto* filament_color =
            dynamic_cast<const ConfigOptionStrings*>(payload->config.option("filament_colour"));
        const auto* nozzle_diameter =
            dynamic_cast<const ConfigOptionFloats*>(payload->config.option("nozzle_diameter"));
        const auto* filament_ids =
            dynamic_cast<const ConfigOptionStrings*>(payload->config.option("filament_ids"));
        const std::string nozzle_diameter_string =
            nozzle_diameter == nullptr ? std::string() : nozzle_diameter->serialize();
        const std::string printer_model_id =
        ...
        for (PlateData* plate : payload->plate_data) {
            ...
            for (auto& filament : plate->slice_filaments_info) {
                std::string display_filament_type;
                filament.type =
                    payload->config.get_filament_type(display_filament_type, filament.id);
    ...
    auto completion_ptr =
        std::make_shared<std::function<void(bool, std::string)>>(std::move(completion));
    auto& worker = get_ui_job_worker();
    const bool queued = queue_job(
        ...
        [payload](Job::Ctl& ctl) {
            ...
            const auto capture_identity = [payload] {
                try {
                    payload->path_identity =
                        Agent::trusted_artifact_identity(payload->path);
                } catch (...) {}
                try {
                    payload->temporary_identity =
                        Agent::trusted_artifact_identity(payload->path + ".tmp");
                } catch (...) {}
            };
            try {
                payload->stored = store_bbs_3mf(params);
            } catch (...) {
                capture_identity();
                throw;
            }
        ...
        [payload, lifetime, completion_ptr](bool canceled, std::exception_ptr& error) mutable {
            std::string message;
            if (error) {
                try {
                    ...
                    message = "Orca project save failed";
                }
                error = nullptr;
            }

            const bool abandoned = lifetime.expired();
            if (abandoned || canceled || !message.empty() || !payload->stored) {
                if (payload->path_identity) {
                    try {
                        Agent::remove_trusted_artifact(
                            payload->path, *payload->path_identity);
                    } catch (...) {}
                }
                if (payload->temporary_identity) {
                    try {
                        Agent::remove_trusted_artifact(
                            payload->path + ".tmp",
                            *payload->temporary_identity);
            ...
            if (abandoned)
                return;
            if (canceled)
                (*completion_ptr)(true, "Orca project save was cancelled");
            else if (!message.empty())
                (*completion_ptr)(true, std::move(message));
            else if (!payload->stored)
                (*completion_ptr)(true, "Orca project save failed");
            else
                (*completion_ptr)(false, {});
        });
